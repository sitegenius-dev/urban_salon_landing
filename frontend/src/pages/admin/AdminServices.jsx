...
import AdminLayout from '../../components/AdminLayout';
import toast from 'react-hot-toast';
import { Plus, Edit2, Trash2, X, ToggleLeft, ToggleRight, ImagePlus } from 'lucide-react';
...
export default function AdminServices() {
  ...
  const openEdit = (svc) => {
    ...
    setImageFile(null);
    setImagePreview(svc.imageUrl ? (svc.imageUrl.startsWith('http') ? svc.imageUrl : `${BASE}${svc.imageUrl}`) : null);
    setModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      // Use FormData so image can be sent as multipart
      const fd = new FormData();
      fd.append('name', form.name.trim());
      fd.append('category', form.category.trim() || '');
      fd.append('description', form.description || '');
      fd.append('price', parseFloat(form.price) || 0);
      fd.append('duration', form.duration ? parseInt(form.duration) : '');
  ...
  const toggleActive = async (svc) => {
    try {
      ...
      await api.put(`/services/${svc.id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      fetchServices();
    } catch { toast.error('Update failed'); }
  };
              ...
              <table className="w-full admin-table">
                ...
                <tbody>
                  {services.map(svc => (
                    <tr key={svc.id} className="text-gray-300">
                      <td>
                        <div className="w-12 h-12 rounded-lg overflow-hidden bg-[#1a1a1a] flex items-center justify-center flex-shrink-0">
                          {svc.imageUrl ? (
                            <img
                              src={svc.imageUrl.startsWith('http') ? svc.imageUrl : `${BASE}${svc.imageUrl}`}
                              alt={svc.name}
                      ...
                      <td>
                        <div className="font-medium text-sm">{svc.name}</div>
                        {svc.description && <div className="text-gray-500 text-xs mt-0.5 max-w-[180px] truncate">{svc.description}</div>}
                      </td>
                      <td>
                        {svc.category
                          ? <span className="bg-[#1a1a1a] text-gold text-xs px-2 py-0.5 rounded-full">{svc.category}</span>
                  ...
                  {services.length === 0 && (
                    <tr><td colSpan={7} className="text-center text-gray-600 py-12">No services yet. Add one!</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <div
                  className="relative w-full h-36 rounded-xl overflow-hidden border-2 border-dashed border-white/10 cursor-pointer hover:border-gold/50 transition-colors flex items-center justify-center bg-[#1a1a1a]"
                ...
                >
                  {imagePreview ? (
                    ...
                    <div className="text-center">
                      <ImagePlus size={24} className="text-gray-600 mx-auto mb-1" />
                      <p className="text-gray-600 text-xs">Click to upload image</p>
                      <p className="text-gray-700 text-[10px] mt-0.5">JPG, PNG, WEBP — max 5MB</p>
                    </div>
                  )}
                </div>
                <input
