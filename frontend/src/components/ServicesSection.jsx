...
//                 {svc.reviewCount && <span className="text-xs text-gray-600">({svc.reviewCount} reviews)</span>}
//               </div>
//             )}
//           </div>
//           {svc.description && (
...
//           <p className="text-xs text-gray-600">After first service delivery</p>
//         </div>
//       </div>
...
//                   <p className="text-xs text-gray-600">₹{parseFloat(svc.price || 0).toFixed(0)}</p>
//                 </div>
//                 <button onClick={() => onRemove(String(svc.id))} className="text-black-300 hover:text-red-400 p-1">
//                   <Trash2 size={18} />
//                 </button>
//               </div>
//             ))}
...
//           <p className="text-xs text-gray-600">No services added</p>
//         </div>
//       )}
//     </div>
//   );
...
//         <p className="text-sm text-gray-700 uppercase tracking-wider font-semibold mb-3">Select a service</p>
...
//             )}
//             {svc.duration && (
//               <span className="text-sm text-gray-600">• {svc.duration} mins</span>
//             )}
//           </div>
...
//           )}
//           {svc.optionCount && (
//             <span className="text-sm text-gray-600">{svc.optionCount} options</span>
//           )}
//         </div>
//       </div>
//     </div>
...
//             )}
//             {svc.duration && <span className="text-xs text-gray-600">• {svc.duration} mins</span>}
//           </div>
...
function CartPanel({ services, selectedIds, onRemove, onProceedToBook }) {
  ...
  return (
    <div className="hidden lg:flex flex-col gap-3 flex-shrink-0 sticky top-[88px]" style={{ width: 280 }}>
          ...
          <div className="divide-y divide-gray-50 max-h-64 overflow-y-auto">
            {items.map(svc => (
              <div key={svc.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-gray-900 truncate">{svc.name}</p>
                  <p className="text-xs text-gray-500">₹{parseFloat(svc.price || 0).toFixed(0)}</p>
                </div>
...
function CategorySidebar({ categories, categoryImages = {}, active, onSelect }) {
  ...
  return (
    <div className="hidden lg:block flex-shrink-0 sticky top-[88px]" style={{ width: 200 }}>
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        <p className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-3">Select a service</p>
        <div className="grid grid-cols-2 gap-x-3 gap-y-4">
          {allCats.map((cat) => {
            const isActive = active === cat;
            const imgSrc = categoryImages[cat];
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {svc.duration && <span className="text-sm text-gray-500">• {svc.duration} mins</span>}
          </div>
          {svc.description && (
            <div className="mt-2">
              {svc.description.split('\n').filter(Boolean).slice(0, 2).map((line, i) => (
        ...
        <div className="flex-shrink-0 flex flex-col items-center gap-1 pt-1">
          ...
          {svc.optionCount && (
            <span className="text-sm text-gray-500">{svc.optionCount} options</span>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="bg-white border-b border-gray-100 last:border-0 py-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            {svc.price > 0 && <span className="text-sm font-bold text-gray-900">₹{parseFloat(svc.price).toFixed(0)}</span>}
            {svc.originalPrice && parseFloat(svc.originalPrice) > parseFloat(svc.price) && (
              <span className="text-xs text-gray-400 line-through">₹{parseFloat(svc.originalPrice).toFixed(0)}</span>
            )}
            {svc.duration && <span className="text-xs text-gray-500">• {svc.duration} mins</span>}
          </div>
          {svc.description && (
            <div className="mt-1.5">
              {svc.description.split('\n').filter(Boolean).slice(0, 2).map((line, i) => (
...
export default function ServicesSection({
  ...
  // Ref for the services list container — used for smooth scroll to top on category change
  ...
  // ── FIX 2: category select → smooth scroll to TOP of list only, not footer ──
  const handleCategorySelect = (cat) => {
    setActive(cat);
    // Scroll the list container itself to top (not window)
    if (listRef.current) {
      listRef.current.scrollTo({ top: 0, behavior: 'smooth' });
    }
    // On mobile, scroll window just enough to show the service list top
    // We scroll to the section, not the bottom
    const section = document.getElementById('services');
    if (section) {
      const navHeight = 56; // mobile nav height
      const top = section.getBoundingClientRect().top + window.scrollY - navHeight - 48; // 48 = tab bar
    ...
    <section id="services" className="bg-white mt-6 md:mt-10"
      ...
      <div className="lg:hidden sticky top-[56px] z-20 bg-white border-b border-gray-100">
        <div className="flex overflow-x-auto px-4" style={{ scrollbarWidth: 'none' }}>
          {tabs.map(tab => (
            <button key={tab} onClick={() => handleCategorySelect(tab)}
              className={`flex-shrink-0 px-4 py-3 text-sm font-semibold border-b-2 transition-all whitespace-nowrap ${
                active === tab ? 'border-violet-600 text-violet-700' : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}>
              {tab}
      ...
      {/* ── FIX 1: proper gap between 3 columns using gap-6 ── */}
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 pt-6">
        <div className="flex gap-6 items-start">
          ...
          {/* CENTER: Service list — ref attached here */}
